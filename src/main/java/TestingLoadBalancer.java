import java.util.*;
import java.util.concurrent.ConcurrentHashMap;
import java.util.stream.IntStream;

public class TestingLoadBalancer {
    HashMap<String, Integer> pool = new HashMap<>();
    private LinkedList<String> p = new LinkedList<String>();

    public static void main(String[] args) {

        int NUM_OF_REQUESTS = 5;
        TestingLoadBalancer client = new TestingLoadBalancer();

//        ArrayList<String> ipPool = new ArrayList <>();
//        ipPool.add("http://localhost:8081/Load-Balancer/test");
//        ipPool.add("http://localhost:8082/Load-Balancer/test");
//        ipPool.add("http://localhost:8083/Load-Balancer/test");
//        ipPool.add("192.168.0.4");
//        ipPool.add("192.168.0.5");

        Map <String, Integer> ipPoolWeighted = new HashMap<>();
        ipPoolWeighted.put("http://localhost:8081/reader/read",  6);
        ipPoolWeighted.put("http://localhost:8082/reader/read",  4);
        ipPoolWeighted.put("http://localhost:8083/reader/read",  3);

//        client.printNextTurn("Random");
//        LoadBalancer random = new RandomLoadBalancer(ipPool);
//        client.simulateConcurrentClientRequest(random, NUM_OF_REQUESTS);
//
//        client.printNextTurn("Round-Robin");
//        LoadBalancer roundRobbin = new RoundRobinLoadBalancer(ipPool);
//        client.simulateConcurrentClientRequest(roundRobbin, NUM_OF_REQUESTS);

        client.printNextTurn("Weighted-Round-Robin");
        LoadBalancer weightedRoundRobin = new WeightedRoundRobinLoadBalancer(ipPoolWeighted);
      //  weightedRoundRobin.getIp();
        //System.out.println("gitIp: "+         weightedRoundRobin.getIp());
         client.simulateConcurrentClientRequest(weightedRoundRobin, NUM_OF_REQUESTS);


    }

    private void simulateConcurrentClientRequest(LoadBalancer loadBalancer, int numOfCalls) {
        IntStream
                .range(0, numOfCalls )
                .parallel()
                .forEach(i ->
                       //  System.out.println(i)
                  //     pool.put(loadBalancer.getIp(),i)
                        p.add(loadBalancer.getIp())

                );
//        for (Object value : pool.keySet()) {
//            // ...
//           System.out.println(value);
//        }
        //p.forEach(e-> System.out.println(e));
        int num = 0;
        while (p.size() > num) {
            System.out.println(p.get(num));
            num++;
        }

    }



//    private void simulateConcurrentClientRequest(LoadBalancer loadBalancer, int numOfCalls) {
//
//        IntStream
//                .range(0, numOfCalls )
//                .parallel()
//                .forEach(i ->
//                        System.out.println(
//                                "IP: " + loadBalancer.getIp()
//                                        + " --- Request from Client: " + i
//                                        + " --- [Thread: " + Thread.currentThread().getName() + "]")
//                );
//    }
//




    private void printNextTurn(String name) {
        System.out.println("---");
        System.out.println("Clients starts to send requests to " + name + " Load Balancer");
        System.out.println("---");
    }

}